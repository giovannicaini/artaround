...
export class WikidataAutocomplete extends LitElement {
  ...
  private selectResult(result: WikidataSearchResult) {
    ...
    this.dispatchEvent(new CustomEvent('wikidata-select', {
      detail: { 
        id: result.id, 
        label: result.label,
      ...
      <div class="relative">
        ...
        <div class="relative">
          <input
            ...
            @input="${(e: Event) => this.handleInput(new CustomEvent('input-change', { detail: { value: (e.target as HTMLInputElement).value } }))}"
          ...
          <div class="absolute inset-y-0 right-0 flex items-center pr-3">
            ${this.loading ? html`
              <svg class="animate-spin h-4 w-4 text-surface-400" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
            ` : this.selectedId ? html`
              <ui-icon name="check" size="xs" class="text-success-500"></ui-icon>
            ` : ''}
          </div>
        </div>
        ...
        ${this.selectedId ? html`
          <div class="mt-1.5 flex items-center gap-2">
            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-brand-100 text-brand-800 dark:bg-brand-900/30 dark:text-brand-300">
        ...
        <!-- Dropdown results -->
        ${this.showDropdown && (this.results.length > 0 || this.loading) ? html`
          <div class="absolute z-50 w-full mt-1 bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-lg shadow-lg max-h-60 overflow-y-auto">
            ${this.loading && this.results.length === 0 ? html`
              <div class="px-4 py-3 text-sm text-surface-500 dark:text-surface-400 flex items-center gap-2">
              ...
              >
                <div class="flex items-start gap-3">
                  <div class="flex-shrink-0 mt-0.5">
                    <ui-icon name="link" size="xs" class="text-surface-400"></ui-icon>
                  </div>
        ...
        ${this.error ? html`
          <p class="mt-1.5 text-xs text-danger-600 dark:text-danger-400 flex items-center gap-1" role="alert">
