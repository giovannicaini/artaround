      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <span class="text-xs text-surface-500">
            ${this.matrix.flat().filter((c) => c.filled).length} / ${this.matrix.flat().length}
            celle
          </span>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full border-collapse">
            <thead>
              <tr>
                <th
                  class="p-2 text-left text-xs font-medium text-surface-500 dark:text-surface-400 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700"
                >
                ...
                ${this.languages.map(
                  (lang) => html`
                    <th
                      class="p-2 text-center text-xs font-medium text-surface-500 dark:text-surface-400 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 min-w-[120px]"
                    >
            ...
            <tbody>
              ${this.durations.map(
                (duration, rowIndex) => html`
                  <tr>
                    <td
                      class="p-2 text-xs font-medium text-surface-700 dark:text-surface-300 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700"
                    >
                      ${duration.label}
                    </td>
                    ${this.languages.map((_, colIndex) => {
                      ...
                      return html`
                        ...
                        >
                          <div
                            class="h-16 flex items-center justify-center ${status === 'filled'
                              ? 'bg-success-50 dark:bg-success-900/20'
                              : 'bg-surface-50 dark:bg-surface-800/50'} rounded-lg"
                          >
                            ${status === 'filled'
                              ? html`
                                  <div class="text-center p-2">
                                    <ui-icon
                                      name="check"
                                      size="sm"
                                      class="text-success-500 mx-auto mb-1"
                                    ></ui-icon>
                                    <p
        ...
        ${this.editingCell
          ? html`
              <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
                <div
                  class="bg-white dark:bg-surface-900 rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"
                >
                  <div
                  ...
                  >
                    <div>
                      ...
                      <p class="text-sm text-surface-500 dark:text-surface-400 mt-0.5">
                        ...
                        ${this.languages[this.editingCell.col].label}
                      </p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div class="p-4 space-y-4">
                    ...
                    <!-- Word count estimate -->
                    <div
                      class="flex items-center gap-4 text-xs text-surface-500 dark:text-surface-400"
                    >
                      <span
                        >~${Math.ceil(this.editText.split(/\s+/).filter((w) => w).length / 150)} min
                        lettura</span
                      >
                  ...
                  <div
                    class="p-4 border-t border-surface-200 dark:border-surface-700 flex justify-end gap-3"
                  >
                    <ui-button
                      variant="secondary"
                      label="Annulla"
