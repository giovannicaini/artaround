...
export class MuseumSelector extends LitElement {
  ...
  private selectMuseum(museum: Museum) {
    ...
    this.dispatchEvent(new CustomEvent('museum-selected', {
      detail: museum,
      bubbles: true,
      composed: true
    }));
  ...
  private renderMuseumCard(museum: Museum) {
    ...
    return html`
      <div 
        class="group cursor-pointer transition-all duration-200 ${isSelected ? 'ring-2 ring-brand-500 ring-offset-2' : ''}"
      ...
      >
        <ui-card padding="none" hover>
          ...
          <div class="aspect-video bg-surface-100 dark:bg-surface-800 relative overflow-hidden rounded-t-xl">
            ${museum.images && museum.images.length > 0 ? html`
              <img 
                src="${museum.images[0]}" 
                alt="${museum.name}"
            ...
            ${isSelected ? html`
              <div class="absolute top-3 right-3 w-8 h-8 rounded-full bg-brand-600 flex items-center justify-center">
          ...
          <!-- Content -->
          <div class="p-4">
            <h3 class="font-semibold text-surface-900 dark:text-white mb-1 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">
              ${museum.name}
            </h3>
            ...
            <div class="flex items-center gap-2 text-xs text-surface-400">
              ...
              <span>${museum.location?.city || 'Città non specificata'}, ${museum.location?.country || ''}</span>
            </div>
          </div>
        </ui-card>
      </div>
  ...
  render() {
    ...
    if (this.museums.length === 0) {
      return html`
        <div class="text-center py-12">
          <ui-icon name="folder" size="lg" class="text-surface-300 dark:text-surface-600 mx-auto mb-4"></ui-icon>
