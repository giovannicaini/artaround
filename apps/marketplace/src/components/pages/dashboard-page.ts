...
export class DashboardPage extends LitElement {
  ...
  private recentArtworks = [
    { id: '1', title: 'Tramonto sul mare', artist: 'Mario Rossi', status: 'approved', date: '2 ore fa' },
    { id: '2', title: 'Ritratto moderno', artist: 'Laura Bianchi', status: 'pending', date: '4 ore fa' },
    { id: '3', title: 'Paesaggio urbano', artist: 'Giovanni Verdi', status: 'approved', date: '1 giorno fa' },
    { id: '4', title: 'Natura morta', artist: 'Anna Neri', status: 'rejected', date: '1 giorno fa' },
    { id: '5', title: 'Composizione astratta', artist: 'Paolo Gialli', status: 'pending', date: '2 giorni fa' },
  ];
      ...
      <ui-card padding="md">
        <div class="flex items-start justify-between">
          <div>
            ...
            <p class="text-2xl font-semibold text-surface-900 dark:text-white mt-1">${stat.value}</p>
          </div>
          <div class="p-2 rounded-lg bg-brand-50 dark:bg-brand-900/30">
            <ui-icon name="${stat.icon}" size="sm" class="text-brand-600 dark:text-brand-400"></ui-icon>
          </div>
        </div>
        <div class="flex items-center gap-1 mt-3">
          <span class="text-sm font-medium ${stat.positive ? 'text-success-600 dark:text-success-400' : 'text-danger-600 dark:text-danger-400'}">
            ${stat.change}
          </span>
  ...
  private getStatusBadge(status: string) {
    ...
    return html`<ui-badge variant="${config.variant}" size="sm" dot label="${config.label}"></ui-badge>`;
  }
      ...
      <div class="space-y-6 animate-fade-in">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 class="text-2xl font-semibold text-surface-900 dark:text-white">
              Buongiorno, ${this.user?.name?.split(' ')[0] || 'Admin'}
            </h2>
                ...
                <table class="w-full">
                  <thead>
                    <tr class="border-b border-surface-200 dark:border-surface-800">
                      <th class="px-5 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Opera</th>
                      <th class="px-5 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider hidden sm:table-cell">Artista</th>
                      <th class="px-5 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider">Stato</th>
                      <th class="px-5 py-3 text-left text-xs font-medium text-surface-500 uppercase tracking-wider hidden md:table-cell">Data</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-surface-200 dark:divide-surface-800">
                    ${this.recentArtworks.map(artwork => html`
                      <tr class="hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                        <td class="px-5 py-4">
                          <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm font-medium text-surface-900 dark:text-white truncate max-w-[150px]">
                              ${artwork.title}
                            </span>
                          </div>
                        </td>
                        <td class="px-5 py-4 hidden sm:table-cell">
                          <span class="text-sm text-surface-600 dark:text-surface-400">${artwork.artist}</span>
                        </td>
                        <td class="px-5 py-4">
                          ${this.getStatusBadge(artwork.status)}
                        </td>
          ...
          <div>
            <ui-card padding="none">
              ...
              <div class="divide-y divide-surface-200 dark:divide-surface-800">
                ${this.recentUsers.map(user => html`
                  <div class="px-5 py-4 flex items-center gap-3">
                    <ui-avatar initials="${user.name.split(' ').map(n => n[0]).join('')}" size="md"></ui-avatar>
            ...
            <ui-card padding="md" class="mt-4">
              ...
              <div class="space-y-2">
                <ui-button variant="secondary" size="sm" block label="Gestisci Opere" icon="image"></ui-button>
                <ui-button variant="secondary" size="sm" block label="Gestisci Utenti" icon="users"></ui-button>
                <ui-button variant="secondary" size="sm" block label="Impostazioni" icon="cog"></ui-button>
              </div>
            </ui-card>
          </div>
        </div>
        ...
        <ui-card padding="md">
          <div class="flex items-center justify-between mb-4">
            <h3 class="font-semibold text-surface-900 dark:text-white">Andamento Visualizzazioni</h3>
          ...
          <div class="h-64 flex items-center justify-center bg-surface-50 dark:bg-surface-800/50 rounded-lg border-2 border-dashed border-surface-200 dark:border-surface-700">
            <div class="text-center">
              <ui-icon name="chart" size="lg" class="text-surface-300 dark:text-surface-600 mx-auto"></ui-icon>
